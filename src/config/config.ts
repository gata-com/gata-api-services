...
export const config = {
  ...
  database: {
    ...
    // SQLite Properties
    database: process.env.SQLITE_DATABASE || './data/student_management.sqlite',
    ...
    name: process.env.DB_NAME || 'gataDB',
  ...
  email: {
    ...
    from: process.env.SMTP_FROM || process.env.SMTP_USER || 'noreply@yourapp.com'
  },
...
if (config.database.type === 'sqlite') {
  console.log(`📁 SQLite database: ${config.database.database}`);
} else {
  ...
  console.log(`⏱️  MySQL connection timeout: ${config.database.connectionTimeout}ms`);
