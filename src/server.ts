...
const checkDatabaseHealth = async (retries = 3): Promise<boolean> => {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.warn(`🔍 Database health check failed (attempt ${i + 1}/${retries}):`, error);
      if (i < retries - 1) {
  ...
  try {
    ...
    const server = app.listen(PORT, () => {
      ...
      if (config.email.user && config.email.password) {
        ...
        console.log('⚠️ SMTP configuration incomplete - email features disabled');
      }
      console.log('============================\n');
    });
      ...
      const monitorInterval = setInterval(async () => {
        try {
          if (!await checkDatabaseHealth(1)) {
            console.error('❌ Database health check failed - connection might be lost');
